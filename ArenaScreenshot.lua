local AS_Sound = true;	--true & false
local AS_Max_Enemies = 1	--有多少目標盯你就報警
...
function EnemyTargetCheck()
	...
	if( nCount>= AS_Max_Enemies ) then
		if( sName == AS_PlayerName ) then
			AS_Frame:Show()
AS_AlertFrame:AddMessage("[你]|cffFFFF40被 "..nCount.." 個敵人盯上!|r")
DEFAULT_CHAT_FRAME:AddMessage("[你]|cffFFFF40被 "..nCount.." 個敵人盯上!|r")
			if AS_Sound then
	--PlaySoundFile(8037)	--塵歸塵，土歸土 ExecutusSlay01
	PlaySoundFile(9278)	--快跑阿小女孩 HoodWolfTransformPlayer01
			end
		else

AS_AlertFrame:AddMessage("|cffFFFF40"..sName.."|r ["..sClass.."]|cffFFFF40被 "..nCount.." 個敵人盯上!|r")
AS_AlertFrame:AddMessage("|cffFFFF40"..sName.."|r ["..sClass.."]|cffFFFF40被 "..nCount.." 個敵人盯上!|r")
		end
	end
end
...
AS_Frame:SetScript("OnEvent", function(self, event, ...)
	...
	elseif (event=="ZONE_CHANGED_NEW_AREA" or event=="PLAYER_ENTERING_WORLD" or event=="UPDATE_BATTLEFIELD_STATUS" ) then
...
AS_AlertFrame:SetPoint("CENTER", 0, -50)
